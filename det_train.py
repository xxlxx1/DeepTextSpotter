...
parser.add_argument('-valid_list', default='/data/xlxia/code/icdar_2017/train.txt')
parser.add_argument('-debug', type=int, default=0)
parser.add_argument('-batch_size', type=int, default=4)  # 32
parser.add_argument('-iterate', type=int, default=20)  # 迭代次数
parser.add_argument('-snapshot_interval', type=int, default=10)  # 每迭代x轮保存
parser.add_argument('-valid_interval', type=int, default=5)  # 每迭代x轮验证
...
def process_batch(nets, optim, optim2, image_size, args):
  ...
  net.blobs['data'].reshape(args.batch_size,1,image_size[1],image_size[0]) # 把一个batch的输入图片reshape
  net.reshape()
...
if __name__ == "__main__":
  ...
  nets[0].net.copy_from('models/tiny.caffemodel')
  #nets[1].net.copy_from('models/model.caffemodel')
  yolonet = nets[0]
  net_ctc = nets[1]
  sgd = net_ctc
  dataloader = DataLoader({'train': args.train_list, 'valid': args.valid_list},root_dir = args.data_dir, gray_scale=True)
